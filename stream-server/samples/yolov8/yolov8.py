...
def yolov8_build_config(algorithm_name,stream_path,data,port):
    ...
    # det_config_path=config_path+'yolov8_group.json'
...
def yolov8_trans_json(json_data,task_id,Type,up_list):
    results={}
    ...
    if len(json_data["mDetectedObjectMetadatas"]) > 0:
        for indx, item in enumerate(json_data["mDetectedObjectMetadatas"]):
            ...
            result = {
                ...
                "Type": Type,
                "Extend": {
                    "类别": yolov8_dict.get(item["mClassify"], 'Not found'),
...
def yolov8_logic(json_data,up_list,rm_list):
